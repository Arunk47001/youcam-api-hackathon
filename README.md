...
## Try before you upload / see more options

Two things beyond the core flow, both scoped after real API research rather than guessed:

- **"See an example"** (`lib/agent/instantDemo.ts`): landing-page buttons that show a full example results page instantly, entirely client-side — no network call, no photo of any person (real or synthetic) at all. It reuses the same mock data and style-engine logic as `MOCK_YOUCAM=1`, just invoked directly in the browser. Clearly labeled "Example only — not your photo or a real analysis" so it's never confused with a real result.
- **"See 3 more looks"**: `pickLooks()` now takes an `excludeIds` list so a second (or third) batch doesn't repeat garments already shown; `/api/style` accepts it as an optional form field. Each click is a real additional Apparel VTO call when running against live credentials, so it's opt-in per user action rather than fetched upfront.
- **Colorway swatch picker**: any look whose garment has a `colorwayGroup` (see `lib/catalog/garments.ts`) gets a color-swatch row on its card, using *real rendered thumbnails* of the user's own photo in each color — not flat color squares. This means every colorway sibling is rendered on the same uploaded photo **upfront**, as part of the initial `/api/style` (or "see more looks") response (`Look.colorwayRenders`, see the note in `app/api/style/route.ts`), not lazily on click. That's a deliberate, explicitly-requested tradeoff: it costs one real Apparel VTO render per color sibling, every time, whether or not the user ever picks it — confirmed against a live account (a 3-colorway garment showed up with all 3 real renders in one response). Clicking a swatch is then just a local state swap (no further API call, no loading spinner) since the image is already fetched. Works in "See an example" mode too, entirely client-side, at no real cost.
- **Background/scene changing was requested but not built**: researched against the hackathon's actual developer API surface (confirmed via a working reference implementation's source, not guesswork) — the granted S2S API covers skin-tone-analysis, skin-analysis, face-attr-analysis, cloth, makeup-vto, hair-transfer, skin-simulation, and look-vto. Perfect Corp. sells a separate "AI Replace" background-generation product, but no evidence it's part of this developer API tier, so it wasn't implemented rather than risk shipping a guessed, unconfirmed integration.
